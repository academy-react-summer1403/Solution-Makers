...
function CourseComments({ setComment, onOpen }) {
  ...
  const items = useMemo(() => {
    ...
    return data?.data.myCommentsDtos
      .sort(
        (a, b) =>
  ...
  return (
    <div className="px-10">
      {data.data.myCommentsDtos.length > 0 ? (
        <Table
          ...
          bottomContent={
            <div className="flex w-full justify-center">
              <Pagination
                ...
                color="primary"
                page={page}
                total={Math.ceil(data.data.myCommentsDtos.length / rowsPerPage)}
        ...
        <p className="text-lg">هنوز برای دوره ای کامنتی ثبت نکردید</p>
      )}
    </div>
  );
}
